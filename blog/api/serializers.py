...
class CommentSerializer(serializers.ModelSerializer):
    # set required to false so we can recognize newly created comments
    #   only newly created comments will not have an id
...
# we only want comments displayed in the detail view (NOT list view)
class PostDetailSerializer(PostSerializer):
    comments = CommentSerializer(many=True)

    def update(self, instance, validated_data):
        # remove comments from data before calling super
        #   we want to handle the comments ourselves
        comments = validated_data.pop("comments")

        instance = super(PostDetailSerializer, self).update(instance, validated_data)

        # save any newly created comments (which will NOT have an id)
